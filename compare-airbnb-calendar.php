...
{
    fwrite(STDERR, "Usage:\n");
    fwrite(STDERR, "  php compare-airbnb-calendar.php <airbnb-report.csv> [options]\n\n");
    fwrite(STDERR, "Options are the same as compare-bookings-calendar.php, except this script reads Airbnb CSV or XLSX reports.\n");
}
...
{
    $refs = [];
    foreach (['Confirmation Code', 'Reference code'] as $column) {
    ...
    if (preg_match_all('/\b[A-Z0-9]{6,12}\b/', $details, $matches)) {
        foreach ($matches[0] as $match) {
            if (str_starts_with($match, 'TA')) {
                $refs[] = $match;
            }
        }
    }
...
{
    $startDate = normalizeAirbnbDate((string) ($booking['Start date'] ?? $booking['Date'] ?? ''));
    $endDate = normalizeAirbnbDate((string) ($booking['End date'] ?? ''));
    $amount = trim((string) ($booking['Gross earnings'] ?? ''));
    if ($amount === '') {
        $amount = trim((string) ($booking['Amount'] ?? ''));
    }
    $currency = trim((string) ($booking['Currency'] ?? ''));
    if ($amount !== '' && $currency !== '') {
        $amount .= ' ' . $currency;
    }
    ...
    return [
        'Action' => 'Booking',
        'Amount' => $amount,
        'City' => detectCity(($booking['Listing'] ?? '') . ' ' . ($booking['Details'] ?? '')),
        'Email' => '',
        'End Date' => $endDate !== '' ? buildEndDate($endDate) : buildEndDate($startDate),
        'Guests' => 1,
        'Name' => $booking['Guest'] ?? '',
        'Phone' => '',
        'Ref' => $booking['Confirmation Code'] ?? '',
        'Source' => 'Airbnb',
...
function normalizeAirbnbDate(string $date): string
{
    $date = trim($date);
    if ($date === '') {
        return '';
    ...
    return $timestamp === false ? $date : date('Y-m-d H:i:s', $timestamp);
}
...
{
    $cities = ['Amsterdam', 'Athens', 'Barcelona', 'Berlin', 'Budapest', 'Hamburg', 'Madrid', 'Malaga', 'Nice', 'Paris', 'Stockholm', 'Uppsala', 'Warsaw'];
